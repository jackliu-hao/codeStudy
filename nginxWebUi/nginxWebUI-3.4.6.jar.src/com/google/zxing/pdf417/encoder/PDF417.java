...
/* 529 */     this.compact = compact;
/* 530 */     this.compaction = Compaction.AUTO;
/* 531 */     this.encoding = null;
/* 532 */     this.minCols = 2;
/* 533 */     this.maxCols = 30;
/* 534 */     this.maxRows = 30;
/* 535 */     this.minRows = 2;
...
/*     */     int n;
/* 573 */     return ((n = c * r - k) > m + 1) ? (n - m - 1) : 0;
/*     */   }
/*     */   
/*     */   private static void encodeChar(int pattern, int len, BarcodeRow logic) {
/* 577 */     int map = 1 << len - 1;
/* 578 */     boolean last = ((pattern & map) != 0);
/* 579 */     int width = 0;
/* 580 */     for (int i = 0; i < len; i++) {
/* 581 */       boolean black = ((pattern & map) != 0);
/* 582 */       if (last == black) {
/* 583 */         width++;
/*     */       } else {
/* 585 */         logic.addBar(last, width);
/*     */         
/* 587 */         last = black;
/* 588 */         width = 1;
/*     */       } 
...
/* 601 */     int idx = 0;
/* 602 */     for (int y = 0; y < r; y++) {
/* 603 */       int left, right, cluster = y % 3;
/* 604 */       logic.startRow();
/* 605 */       encodeChar(130728, 17, logic.getCurrentRow());
/*     */ 
/*     */ 
/*     */       
/* 609 */       if (cluster == 0) {
/* 610 */         left = 30 * y / 3 + (r - 1) / 3;
/* 611 */         right = 30 * y / 3 + c - 1;
/* 612 */       } else if (cluster == 1) {
/* 613 */         left = 30 * y / 3 + errorCorrectionLevel * 3 + (r - 1) % 3;
/* 614 */         right = 30 * y / 3 + (r - 1) / 3;
/*     */       } else {
/* 616 */         left = 30 * y / 3 + c - 1;
/* 617 */         right = 30 * y / 3 + errorCorrectionLevel * 3 + (r - 1) % 3;
/*     */       } 
/*     */ 
/*     */       
/* 621 */       encodeChar(CODEWORD_TABLE[cluster][left], 17, logic.getCurrentRow());
/*     */       
/* 623 */       for (int x = 0; x < c; x++) {
/*     */         
/* 625 */         encodeChar(CODEWORD_TABLE[cluster][fullCodewords.charAt(idx)], 17, logic.getCurrentRow());
/* 626 */         idx++;
/*     */       } 
/*     */       
/* 629 */       if (this.compact) {
/* 630 */         encodeChar(260649, 1, logic.getCurrentRow());
/*     */       } else {
/*     */         
/* 633 */         encodeChar(CODEWORD_TABLE[cluster][right], 17, logic.getCurrentRow());
/*     */         
/* 635 */         encodeChar(260649, 18, logic.getCurrentRow());
/*     */       } 
/*     */     } 
/*     */   }
/*     */ 
...
/* 648 */     int errorCorrectionCodeWords = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(errorCorrectionLevel);
/*     */     String highLevel;
/* 650 */     int sourceCodeWords = (highLevel = PDF417HighLevelEncoder.encodeHighLevel(msg, this.compaction, this.encoding)).length();
/*     */ 
/*     */ 
/*     */     
/* 654 */     int dimension[], cols = (dimension = determineDimensions(sourceCodeWords, errorCorrectionCodeWords))[0];
/* 655 */     int rows = dimension[1];
/*     */     
/* 657 */     int pad = getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows);
/*     */ 
/*     */     
/* 660 */     if (sourceCodeWords + errorCorrectionCodeWords + 1 > 929) {
/* 661 */       throw new WriterException("Encoded message contains too many code words, message too big (" + msg
/* 662 */           .length() + " bytes)");
/*     */     }
/* 664 */     int n = sourceCodeWords + pad + 1;
/*     */     StringBuilder sb;
/* 666 */     (sb = new StringBuilder(n)).append((char)n);
/* 667 */     sb.append(highLevel);
/* 668 */     for (int i = 0; i < pad; i++) {
/* 669 */       sb.append('΄');
/*     */     }
/*     */ 
/*     */ 
/*     */     
/* 674 */     String dataCodewords, ec = PDF417ErrorCorrection.generateErrorCorrection(dataCodewords = sb.toString(), errorCorrectionLevel);
/*     */ 
/*     */     
/* 677 */     this.barcodeMatrix = new BarcodeMatrix(rows, cols);
/* 678 */     encodeLowLevel(dataCodewords + ec, cols, rows, errorCorrectionLevel, this.barcodeMatrix);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 690 */     float ratio = 0.0F;
/* 691 */     int[] dimension = null;
/*     */     
/* 693 */     for (int cols = this.minCols; cols <= this.maxCols;) {
/*     */ 
/*     */ 
/*     */       
/* 697 */       if ((rows = calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, cols)) >= this.minRows) {
/*     */ 
/*     */ 
/*     */         
/* 701 */         if (rows <= this.maxRows) {
/*     */ 
/*     */ 
/*     */           
/* 705 */           float newRatio = (cols * 17 + 69) * 0.357F / rows * 2.0F;
/*     */ 
/*     */           
/* 708 */           if (dimension == null || Math.abs(newRatio - 3.0F) <= Math.abs(ratio - 3.0F)) {
/*     */ 
/*     */ 
/*     */             
/* 712 */             ratio = newRatio;
/* 713 */             dimension = new int[] { cols, rows };
/*     */           } 
/*     */         }  cols++;
/*     */       } 
/* 717 */     }  if (dimension == null && 
/* 718 */       calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, this.minCols) < 
/* 719 */       this.minRows) {
/* 720 */       dimension = new int[] { this.minCols, this.minRows };
/*     */     }
/*     */ 
/*     */     
/* 724 */     if (dimension == null) {
/* 725 */       throw new WriterException("Unable to fit message in columns");
/*     */     }
/*     */     
...
/* 740 */     this.maxCols = maxCols;
/* 741 */     this.minCols = minCols;
/* 742 */     this.maxRows = maxRows;
/* 743 */     this.minRows = minRows;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 750 */     this.compaction = compaction;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 764 */     this.encoding = encoding;
/*     */   }
/*     */ }


/* Location:              G:\git\codeReviewLog\nginxWebUi\nginxWebUI-3.4.6.jar!\com\google\zxing\pdf417\encoder\PDF417.class
