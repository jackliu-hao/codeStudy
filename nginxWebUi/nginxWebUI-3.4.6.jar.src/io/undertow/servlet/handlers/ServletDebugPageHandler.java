/*     */ package io.undertow.servlet.handlers;
/*     */ 
/*     */ import io.undertow.server.HttpServerExchange;
/*     */ import io.undertow.servlet.spec.HttpServletRequestImpl;
/*     */ import java.io.IOException;
/*     */ import java.io.PrintWriter;
/*     */ import java.io.StringWriter;
/*     */ import java.nio.charset.StandardCharsets;
/*     */ import javax.servlet.ServletOutputStream;
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ public class ServletDebugPageHandler
/*     */ {
/*     */   public static final String ERROR_CSS = "<style>\nbody {\n    font-family: \"Lucida Grande\", \"Lucida Sans Unicode\", \"Trebuchet MS\", Helvetica, Arial, Verdana, sans-serif;\n    margin: 5px;\n}\n\n.header {\n    background-image: linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    background-image: -o-linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    background-image: -moz-linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    background-image: -webkit-linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    background-image: -ms-linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    \n    background-image: -webkit-gradient(\n        linear,\n        left bottom,\n        left top,\n        color-stop(0.08, rgb(153,151,153)),\n        color-stop(0.54, rgb(199,199,199))\n    );\n    color: black;\n    padding: 2px;\n    font-weight: normal;\n    border: solid 1px;\n    font-size: 170%;\n    text-align: left;\n    vertical-align: middle; \n    height: 32px; \n    margin-bottom: 10px;\n}\n.error-div {\n    display: inline-block;\n    width: 32px;\n    height: 32px;\n    background: url('data:image/png;base64,') left center no-repeat;\n}\n.error-text-div {\n    display: inline-block;\n    vertical-align: top;\n    height: 32px;\n}\n.label {\n    font-weight:bold;\n    display: inline-block;\n}\n.value {\n    display: inline-block;\n    margin-left: 5px;\n}\npre {\n    font-size: 110%;\n    margin-left: 1.5em;\n    white-space: pre-wrap;\n    white-space: -moz-pre-wrap;\n    white-space: -pre-wrap;\n    white-space: -o-pre-wrap;\n    word-wrap: break-word;\n}\n</style>";
/*     */   
/*     */   public static void handleRequest(HttpServerExchange exchange, ServletRequestContext servletRequestContext, Throwable exception) throws IOException {
/* 101 */     HttpServletRequestImpl req = servletRequestContext.getOriginalRequest();
/* 102 */     StringBuilder sb = new StringBuilder();
/*     */     
/* 104 */     sb.append("<html><head><title>ERROR</title>");
/* 105 */     sb.append("<style>\nbody {\n    font-family: \"Lucida Grande\", \"Lucida Sans Unicode\", \"Trebuchet MS\", Helvetica, Arial, Verdana, sans-serif;\n    margin: 5px;\n}\n\n.header {\n    background-image: linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    background-image: -o-linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    background-image: -moz-linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    background-image: -webkit-linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    background-image: -ms-linear-gradient(bottom, rgb(153,151,153) 8%, rgb(199,199,199) 54%);\n    \n    background-image: -webkit-gradient(\n        linear,\n        left bottom,\n        left top,\n        color-stop(0.08, rgb(153,151,153)),\n        color-stop(0.54, rgb(199,199,199))\n    );\n    color: black;\n    padding: 2px;\n    font-weight: normal;\n    border: solid 1px;\n    font-size: 170%;\n    text-align: left;\n    vertical-align: middle; \n    height: 32px; \n    margin-bottom: 10px;\n}\n.error-div {\n    display: inline-block;\n    width: 32px;\n    height: 32px;\n    background: url('data:image/png;base64,') left center no-repeat;\n}\n.error-text-div {\n    display: inline-block;\n    vertical-align: top;\n    height: 32px;\n}\n.label {\n    font-weight:bold;\n    display: inline-block;\n}\n.value {\n    display: inline-block;\n    margin-left: 5px;\n}\npre {\n    font-size: 110%;\n    margin-left: 1.5em;\n    white-space: pre-wrap;\n    white-space: -moz-pre-wrap;\n    white-space: -pre-wrap;\n    white-space: -o-pre-wrap;\n    word-wrap: break-word;\n}\n</style>");
/* 106 */     sb.append("</head><body><div class=\"header\"><div class=\"error-div\"></div><div class=\"error-text-div\">Error processing request</div></div>");
/* 107 */     writeLabel(sb, "Context Path", req.getContextPath());
/* 108 */     writeLabel(sb, "Servlet Path", req.getServletPath());
/* 109 */     writeLabel(sb, "Path Info", req.getPathInfo());
/* 110 */     writeLabel(sb, "Query String", req.getQueryString());
/* 111 */     writeLabel(sb, "Stack Trace", "");
/*     */     
/* 113 */     sb.append("<pre>");
/* 114 */     StringWriter stringWriter = new StringWriter();
/* 115 */     exception.printStackTrace(new PrintWriter(stringWriter));
/* 116 */     sb.append(escapeBodyText(stringWriter.toString()));
/* 117 */     sb.append("</pre></body></html>");
/* 118 */     servletRequestContext.getOriginalResponse().setContentType("text/html");
/* 119 */     servletRequestContext.getOriginalResponse().setCharacterEncoding("UTF-8");
/*     */     try {
/* 121 */       ServletOutputStream out = servletRequestContext.getOriginalResponse().getOutputStream();
/* 122 */       out.write(sb.toString().getBytes(StandardCharsets.UTF_8));
/* 123 */       out.close();
/* 124 */     } catch (IllegalStateException e) {
/* 125 */       PrintWriter writer = servletRequestContext.getOriginalResponse().getWriter();
/* 126 */       writer.write(sb.toString());
/* 127 */       writer.close();
/*     */     } 
/*     */   }
/*     */   
/*     */   private static void writeLabel(StringBuilder sb, String label, String value) {
/* 132 */     sb.append("<div class=\"label\">");
/* 133 */     sb.append(escapeBodyText(label));
/* 134 */     sb.append(":</div><div class=\"value\">");
/* 135 */     sb.append(escapeBodyText(value));
/* 136 */     sb.append("</div><br/>");
/*     */   }
/*     */   
/*     */   public static String escapeBodyText(String bodyText) {
/* 140 */     if (bodyText == null) {
/* 141 */       return "null";
/*     */     }
/* 143 */     return bodyText.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;");
/*     */   }
/*     */ }


/* Location:              G:\git\codeReviewLog\nginxWebUi\nginxWebUI-3.4.6.jar!\i\\undertow\servlet\handlers\ServletDebugPageHandler.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */