...
/* 183 */     return crypt(original, (String)null);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 203 */     if (salt == null) {
/* 204 */       ThreadLocalRandom randomGenerator = ThreadLocalRandom.current();
/* 205 */       int numSaltChars = SALT_CHARS.length;
/*     */       
/* 207 */       salt = "" + SALT_CHARS[randomGenerator.nextInt(numSaltChars)] + SALT_CHARS[randomGenerator.nextInt(numSaltChars)];
/* 208 */     } else if (!salt.matches("^[./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz]{2,}$")) {
/* 209 */       throw new IllegalArgumentException("Invalid salt value: " + salt);
/*     */     } 
/*     */     
/* 212 */     StringBuilder buffer = new StringBuilder("             ");
/* 213 */     char charZero = salt.charAt(0);
/* 214 */     char charOne = salt.charAt(1);
/* 215 */     buffer.setCharAt(0, charZero);
/* 216 */     buffer.setCharAt(1, charOne);
/* 217 */     int eSwap0 = CON_SALT[charZero];
/* 218 */     int eSwap1 = CON_SALT[charOne] << 4;
/* 219 */     byte[] key = new byte[8]; int i;
/* 220 */     for (i = 0; i < key.length; i++) {
/* 221 */       key[i] = 0;
/*     */     }
/*     */     
/* 224 */     for (i = 0; i < key.length && i < original.length; i++) {
/* 225 */       int iChar = original[i];
/* 226 */       key[i] = (byte)(iChar << 1);
/*     */     } 
/*     */     
/* 229 */     int[] schedule = desSetKey(key);
/* 230 */     int[] out = body(schedule, eSwap0, eSwap1);
/* 231 */     byte[] b = new byte[9];
...
/* 249 */           u = 128;
/*     */         } 
/* 251 */         buffer.setCharAt(j, (char)COV2CHAR[c]);
/*     */       } 
/*     */     } 
...
/* 305 */     left = t >>> 1 | t << 31;
/* 306 */     int[] results = new int[2];
/* 307 */     permOp(right, left, 1, 1431655765, results);
/* 308 */     right = results[0];
/* 309 */     left = results[1];
/* 310 */     permOp(left, right, 8, 16711935, results);
/* 311 */     left = results[0];
/* 312 */     right = results[1];
/* 313 */     permOp(right, left, 2, 858993459, results);
/* 314 */     right = results[0];
/* 315 */     left = results[1];
/* 316 */     permOp(left, right, 16, 65535, results);
/* 317 */     left = results[0];
/* 318 */     right = results[1];
/* 319 */     permOp(right, left, 4, 252645135, results);
/* 320 */     right = results[0];
/* 321 */     left = results[1];
/* 322 */     int[] out = new int[2];
/* 323 */     out[0] = left;
/* 324 */     out[1] = right;
/* 325 */     return out;
/*     */   }
...
/* 339 */     t = t >>> 4 | t << 28;
/* 340 */     el ^= SPTRANS[1][t & 0x3F] | SPTRANS[3][t >>> 8 & 0x3F] | SPTRANS[5][t >>> 16 & 0x3F] | SPTRANS[7][t >>> 24 & 0x3F] | SPTRANS[0][u & 0x3F] | SPTRANS[2][u >>> 8 & 0x3F] | SPTRANS[4][u >>> 16 & 0x3F] | SPTRANS[6][u >>> 24 & 0x3F];
/*     */ 
/*     */     
/* 343 */     return el;
/*     */   }
...
/* 347 */     int[] schedule = new int[32];
/* 348 */     int c = fourBytesToInt(key, 0);
/* 349 */     int d = fourBytesToInt(key, 4);
/* 350 */     int[] results = new int[2];
/* 351 */     permOp(d, c, 4, 252645135, results);
/* 352 */     d = results[0];
/* 353 */     c = results[1];
/* 354 */     c = hPermOp(c, -2, -859045888);
/* 355 */     d = hPermOp(d, -2, -859045888);
/* 356 */     permOp(d, c, 1, 1431655765, results);
/* 357 */     d = results[0];
/* 358 */     c = results[1];
/* 359 */     permOp(c, d, 8, 16711935, results);
/* 360 */     c = results[0];
/* 361 */     d = results[1];
/* 362 */     permOp(d, c, 1, 1431655765, results);
/* 363 */     d = results[0];
/* 364 */     c = results[1];
...
/*     */ 
/*     */       
/* 381 */       int t = SKB[4][d & 0x3F] | SKB[5][d >>> 7 & 0x3 | d >>> 8 & 0x3C] | SKB[6][d >>> 15 & 0x3F] | SKB[7][d >>> 21 & 0xF | d >>> 22 & 0x30];
/*     */       
...
/* Location:              G:\git\codeReviewLog\nginxWebUi\nginxWebUI-3.4.6.jar!\org\apache\commons\codec\digest\UnixCrypt.class
