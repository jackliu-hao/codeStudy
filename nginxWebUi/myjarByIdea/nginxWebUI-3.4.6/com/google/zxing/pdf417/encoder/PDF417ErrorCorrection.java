...
final class PDF417ErrorCorrection {
   ...
   static String generateErrorCorrection(CharSequence dataCodewords, int errorCorrectionLevel) {
      int k;
      char[] e = new char[k = getErrorCorrectionCodewordCount(errorCorrectionLevel)];
      int sld = dataCodewords.length();

      int j;
      for(int i = 0; i < sld; ++i) {
         j = (dataCodewords.charAt(i) + e[k - 1]) % 929;

         int t2;
         int t3;
         for(int j = k - 1; j > 0; --j) {
            t2 = j * EC_COEFFICIENTS[errorCorrectionLevel][j] % 929;
            t3 = 929 - t2;
            e[j] = (char)((e[j - 1] + t3) % 929);
         }

         t2 = j * EC_COEFFICIENTS[errorCorrectionLevel][0] % 929;
         t3 = 929 - t2;
         e[0] = (char)(t3 % 929);
      }
