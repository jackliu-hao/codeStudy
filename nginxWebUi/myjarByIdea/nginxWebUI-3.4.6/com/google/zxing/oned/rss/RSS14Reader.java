...
public final class RSS14Reader extends AbstractRSSReader {
   ...
   public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints) throws NotFoundException {
      ...
      row.reverse();
      Iterator var6 = this.possibleLeftPairs.iterator();
      ...
      while(true) {
         ...
         } while((left = (Pair)var6.next()).getCount() <= 1);

         Iterator var8 = this.possibleRightPairs.iterator();

         while(var8.hasNext()) {
            Pair right;
            if ((right = (Pair)var8.next()).getCount() > 1 && checkChecksum(left, right)) {
   ...
   private static void addOrTally(Collection<Pair> possiblePairs, Pair pair) {
      if (pair != null) {
         boolean found = false;
         Iterator var3 = possiblePairs.iterator();

         while(var3.hasNext()) {
            Pair other;
            if ((other = (Pair)var3.next()).getValue() == pair.getValue()) {
   ...
   private static boolean checkChecksum(Pair leftPair, Pair rightPair) {
      ...
      if ((targetCheckValue = 9 * leftPair.getFinderPattern().getValue() + rightPair.getFinderPattern().getValue()) > 72) {
         --targetCheckValue;
      }
   ...
   private DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern, boolean outsideChar) throws NotFoundException {
      ...
      for(oddSum = 0; oddSum < counters.length; ++oddSum) {
         float value;
         if ((evenChecksumPortion = (int)((value = (float)counters[oddSum] / elementWidth) + 0.5F)) <= 0) {
      ...
      for(evenChecksumPortion = oddCounts.length - 1; evenChecksumPortion >= 0; --evenChecksumPortion) {
         oddChecksumPortion = oddChecksumPortion * 9 + oddCounts[evenChecksumPortion];
      ...
      for(checksumPortion = evenCounts.length - 1; checksumPortion >= 0; --checksumPortion) {
         evenChecksumPortion = evenChecksumPortion * 9 + evenCounts[checksumPortion];
