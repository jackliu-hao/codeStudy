...
final class DecoderConfig {
   ...
   Map<DecodeHintType, ?> buildHints() {
      ...
      if (finalPossibleFormats == null || ((List)finalPossibleFormats).isEmpty()) {
         finalPossibleFormats = new ArrayList();
         ((List)finalPossibleFormats).addAll(Arrays.asList(BarcodeFormat.UPC_A, BarcodeFormat.UPC_E, BarcodeFormat.EAN_13, BarcodeFormat.EAN_8, BarcodeFormat.RSS_14, BarcodeFormat.RSS_EXPANDED));
         if (!this.productsOnly) {
            ((List)finalPossibleFormats).addAll(Arrays.asList(BarcodeFormat.CODE_39, BarcodeFormat.CODE_93, BarcodeFormat.CODE_128, BarcodeFormat.ITF, BarcodeFormat.QR_CODE, BarcodeFormat.DATA_MATRIX, BarcodeFormat.AZTEC, BarcodeFormat.PDF_417, BarcodeFormat.CODABAR, BarcodeFormat.MAXICODE));
         }
      }
