...
final class DecodedBitStreamParser {
   ...
   private static Mode decodeAsciiSegment(BitSource bits, StringBuilder result, StringBuilder resultTrailer) throws FormatException {
      ...
      int oneByte;
      while((oneByte = bits.readBits(8)) != 0) {
         if (oneByte <= 128) {
            if (upperShift) {
               oneByte += 128;
            }
   ...
   private static void decodeC40Segment(BitSource bits, StringBuilder result) throws FormatException {
      ...
      do {
         ...
         int firstByte;
         if ((firstByte = bits.readBits(8)) == 254) {
            return;
         }
   ...
   private static void parseTwoBytes(int firstByte, int secondByte, int[] result) {
      int fullBitValue;
      int temp = (fullBitValue = (firstByte << 8) + secondByte - 1) / 1600;
      result[0] = temp;
      temp = (fullBitValue -= temp * 1600) / 40;
      result[1] = temp;
   ...
   private static void decodeEdifactSegment(BitSource bits, StringBuilder result) {
      while(bits.available() > 16) {
         for(int i = 0; i < 4; ++i) {
            int edifactValue;
            if ((edifactValue = bits.readBits(6)) == 31) {
               int bitsLeft;
               if ((bitsLeft = 8 - bits.getBitOffset()) != 8) {
   ...
   private static void decodeBase256Segment(BitSource bits, StringBuilder result, Collection<byte[]> byteSegments) throws FormatException {
      ...
      int d1;
      int count;
      if ((d1 = unrandomize255State(bits.readBits(8), codewordPosition++)) == 0) {
   ...
   private static int unrandomize255State(int randomizedBase256Codeword, int base256CodewordPosition) {
      ...
      int tempVariable;
      return (tempVariable = randomizedBase256Codeword - pseudoRandomNumber) >= 0 ? tempVariable : tempVariable + 256;
   }
