...
public final class Detector {
   ...
   public DetectorResult detect() throws NotFoundException {
      ...
      ResultPoint pointA = (cornerPoints = this.rectangleDetector.detect())[0];
      ResultPoint pointB = cornerPoints[1];
      ResultPoint pointC = cornerPoints[2];
      ResultPoint pointD = cornerPoints[3];
      ArrayList transitions;
      (transitions = new ArrayList(4)).add(this.transitionsBetween(pointA, pointB));
      ...
      HashMap pointCount;
      increment(pointCount = new HashMap(), lSideOne.getFrom());
      ...
      Iterator var13 = pointCount.entrySet().iterator();

      ResultPoint topLeft;
      while(var13.hasNext()) {
         Map.Entry entry;
         topLeft = (ResultPoint)(entry = (Map.Entry)var13.next()).getKey();
      ...
      if (maybeTopLeft != null && bottomLeft != null && maybeBottomRight != null) {
         ResultPoint[] corners;
         ResultPoint.orderBestPatterns(corners = new ResultPoint[]{maybeTopLeft, bottomLeft, maybeBottomRight});
         ...
         if (4 * dimensionTop < dimensionRight * 7 && 4 * dimensionRight < dimensionTop * 7) {
            int dimension = Math.min(dimensionRight, dimensionTop);
            if ((correctedTopRight = this.correctTopRight(bottomLeft, bottomRight, topLeft, topRight, dimension)) == null) {
            ...
            if ((correctedTopRight = this.correctTopRightRectangular(bottomLeft, bottomRight, topLeft, topRight, dimensionTop, dimensionRight)) == null) {
               correctedTopRight = topRight;
            }
   ...
   private ResultPointsAndTransitions transitionsBetween(ResultPoint from, ResultPoint to) {
      ...
      boolean steep;
      int dx;
      if (steep = Math.abs((toY = (int)to.getY()) - fromY) > Math.abs(toX - fromX)) {
         dx = fromX;
         fromX = fromY;
         fromY = dx;
         dx = toX;
      ...
      for(int y = fromY; x != toX; x += xstep) {
         boolean isBlack;
         if ((isBlack = this.image.get(steep ? y : x, steep ? x : y)) != inBlack) {
            ++transitions;
            inBlack = isBlack;
         }
