...
final class AlignmentPatternFinder {
   ...
   private boolean foundPatternCross(int[] stateCount) {
      float moduleSize;
      float maxVariance = (moduleSize = this.moduleSize) / 2.0F;
   ...
   private float crossCheckVertical(int startI, int centerJ, int maxCount, int originalStateCountTotal) {
      ...
      int[] stateCount;
      (stateCount = this.crossCheckStateCount)[0] = 0;
      stateCount[1] = 0;
      stateCount[2] = 0;
   ...
   private AlignmentPattern handlePossibleCenter(int[] stateCount, int i, int j) {
      ...
      float centerI;
      if (!Float.isNaN(centerI = this.crossCheckVertical(i, (int)centerJ, 2 * stateCount[1], stateCountTotal))) {
         float estimatedModuleSize = (float)(stateCount[0] + stateCount[1] + stateCount[2]) / 3.0F;
         Iterator var8 = this.possibleCenters.iterator();

         while(var8.hasNext()) {
            AlignmentPattern center;
            if ((center = (AlignmentPattern)var8.next()).aboutEquals(estimatedModuleSize, centerI, centerJ)) {
