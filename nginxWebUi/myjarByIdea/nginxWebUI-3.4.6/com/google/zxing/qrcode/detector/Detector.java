...
public class Detector {
   ...
   protected final DetectorResult processFinderPatternInfo(FinderPatternInfo info) throws NotFoundException, FormatException {
      ...
      float moduleSize;
      if ((moduleSize = this.calculateModuleSize(topLeft, topRight, bottomLeft)) < 1.0F) {
         throw NotFoundException.getNotFoundInstance();
      } else {
         int dimension;
         Version provisionalVersion;
         int modulesBetweenFPCenters = (provisionalVersion = Version.getProvisionalVersionForDimension(dimension = computeDimension(topLeft, topRight, bottomLeft, moduleSize))).getDimensionForVersion() - 7;
   ...
   private static int computeDimension(ResultPoint topLeft, ResultPoint topRight, ResultPoint bottomLeft, float moduleSize) throws NotFoundException {
      ...
      int dimension;
      switch ((dimension = (tltrCentersDimension + tlblCentersDimension) / 2 + 7) & 3) {
         case 0:
            ++dimension;
         case 1:
         default:
   ...
   private float sizeOfBlackWhiteBlackRunBothWays(int fromX, int fromY, int toX, int toY) {
      ...
      float scale = 1.0F;
      int var10001 = toX - fromX;
      int var10000 = fromX - (toX - fromX);
      int otherToX = fromX - var10001;
      if (var10000 < 0) {
   ...
   private float sizeOfBlackWhiteBlackRun(int fromX, int fromY, int toX, int toY) {
      boolean steep;
      int dx;
      if (steep = Math.abs(toY - fromY) > Math.abs(toX - fromX)) {
         dx = fromX;
         fromX = fromY;
         fromY = dx;
         dx = toX;
   ...
   protected final AlignmentPattern findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY, float allowanceFactor) throws NotFoundException {
      ...
      if ((float)((alignmentAreaRightX = Math.min(this.image.getWidth() - 1, estAlignmentX + allowance)) - alignmentAreaLeftX) < overallEstModuleSize * 3.0F) {
         throw NotFoundException.getNotFoundInstance();
      } else {
         int alignmentAreaTopY = Math.max(0, estAlignmentY - allowance);
         int alignmentAreaBottomY;
         if ((float)((alignmentAreaBottomY = Math.min(this.image.getHeight() - 1, estAlignmentY + allowance)) - alignmentAreaTopY) < overallEstModuleSize * 3.0F) {
            throw NotFoundException.getNotFoundInstance();
         } else {
