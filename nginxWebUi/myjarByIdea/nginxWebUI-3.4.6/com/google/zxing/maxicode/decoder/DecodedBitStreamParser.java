...
final class DecodedBitStreamParser {
      ...
      switch (mode) {
         ...
         case 3:
            ...
            if (mode == 2) {
               ...
               postcode = (new DecimalFormat("0000000000".substring(0, getPostCode2Length(bytes)))).format((long)pc);
            } else {
               postcode = getPostCode3(bytes);
            }

            DecimalFormat threeDigits;
            String country = (threeDigits = new DecimalFormat("000")).format((long)getCountry(bytes));
   ...
   private static String getMessage(byte[] bytes, int start, int len) {
      ...
      for(int i = start; i < start + len; ++i) {
         char c;
         switch (c = SETS[set].charAt(bytes[i])) {
            case '\ufff0':
            case '\ufff1':
            case '\ufff2':
            case '\ufff3':
