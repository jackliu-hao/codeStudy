...
public abstract class DragHandler implements DragSourceListener, DragSourceMotionListener, DragGestureListener {
   ...
   protected boolean canDrag(DragGestureEvent e) {
      ...
      } else if (mods == COPY_MASK) {
         return (this.supportedActions & 1) != 0;
      } else if (mods == LINK_MASK) {
         return (this.supportedActions & 1073741824) != 0;
      } else {
         return true;
      }
   }
      ...
      if ((e.getDragAction() & this.supportedActions) != 0 && this.canDrag(e)) {
         ...
         try {
            ...
            if (dragIcon != null && DragSource.isDragImageSupported()) {
               ...
               e.startDrag((Cursor)cursor, this.createDragImage(gc, dragIcon), this.imageOffset, transferable, this);
            } else {
               if (dragIcon != null) {
               ...
               e.startDrag((Cursor)cursor, transferable, this);
            }
   ...
   private int reduce(int actions) {
      if ((actions & 2) != 0 && actions != 2) {
         ...
         return (actions & 1) != 0 && actions != 1 ? 1 : actions;
      }
   }
