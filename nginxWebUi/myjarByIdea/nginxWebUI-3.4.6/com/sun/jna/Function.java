               ...
               if (inArg instanceof Structure) {
                  ...
                  if (args[i] instanceof PointerArray) {
                     ...
                     if (Structure.ByReference[].class.isAssignableFrom(inArg.getClass())) {
                        Class<? extends Structure> type = inArg.getClass().getComponentType();
                        Structure[] ss = (Structure[])((Structure[])inArg);

                        for(int si = 0; si < ss.length; ++si) {
                           Pointer p = array.getPointer((long)(Native.POINTER_SIZE * si));
   ...
   private static class PointerArray extends Memory implements PostCallRead {
      ...
      public PointerArray(Pointer[] arg) {
         ...
         for(int i = 0; i < arg.length; ++i) {
            this.setPointer((long)(i * Native.POINTER_SIZE), arg[i]);
         }

         this.setPointer((long)(Native.POINTER_SIZE * arg.length), (Pointer)null);
      }
