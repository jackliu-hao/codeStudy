...
public class LogFactoryImpl extends LogFactory {
   ...
   private Log createLogFromClass(String logAdapterClassName, String logCategory, boolean affectState) throws LogConfigurationException {
      ...
      while(true) {
         this.logDiagnostic("Trying to load '" + logAdapterClassName + "' from classloader " + objectId(currentCL));
         ...
         try {
            ...
            try {
               ...
               this.logDiagnostic("The log adapter '" + logAdapterClassName + "' is not available via classloader " + objectId(currentCL) + ": " + msg.trim());
            ...
            this.logDiagnostic("The log adapter '" + logAdapterClassName + "' is missing dependencies when loaded via classloader " + objectId(currentCL) + ": " + resourceName.trim());
            break;
         } catch (ExceptionInInitializerError var17) {
            resourceName = var17.getMessage();
            this.logDiagnostic("The log adapter '" + logAdapterClassName + "' is unable to initialize itself when loaded via classloader " + objectId(currentCL) + ": " + resourceName.trim());
            break;
         } catch (LogConfigurationException var18) {
            throw var18;
         } catch (Throwable var19) {
            handleThrowable(var19);
      ...
      if (logAdapterClass != null && affectState) {
         ...
         try {
            ...
            this.logMethod = null;
            this.logDiagnostic("[INFO] '" + logAdapterClassName + "' from classloader " + objectId(currentCL) + " does not declare optional method " + "setLogFactory(LogFactory)");
         }

         this.logDiagnostic("Log adapter '" + logAdapterClassName + "' from classloader " + objectId(logAdapterClass.getClassLoader()) + " has been selected for use.");
      }
   ...
   private ClassLoader getLowestClassLoader(ClassLoader c1, ClassLoader c2) {
      if (c1 == null) {
         ...
         ClassLoader current;
         for(current = c1; current != null; current = this.getParentClassLoader(current)) {
            if (current == c2) {
               return c1;
            }
         }

         for(current = c2; current != null; current = this.getParentClassLoader(current)) {
            if (current == c1) {
               return c2;
            }
         }
      ...
      if (implementsLog) {
         if (isDiagnosticsEnabled()) {
            try {
               ...
               this.logDiagnostic("Class '" + badClass.getName() + "' was found in classloader " + objectId(badClassLoader) + ". It is bound to a Log interface which is not" + " the one loaded from classloader " + objectId(logInterfaceClassLoader));
            } catch (Throwable var7) {
               handleThrowable(var7);
