...
public class IntList {
   ...
   private void growAtEnd(int required) {
      ...
      if (this.firstIndex >= required - (this.array.length - this.lastIndex)) {
         increment = this.lastIndex - this.firstIndex;
         if (size > 0) {
            System.arraycopy(this.array, this.firstIndex, this.array, 0, size);
         }

         this.firstIndex = 0;
         this.lastIndex = increment;
      } else {
         increment = size / 2;
   ...
   private void growAtFront(int required) {
      ...
      if (this.array.length - this.lastIndex + this.firstIndex >= required) {
         increment = this.array.length - size;
         if (size > 0) {
            System.arraycopy(this.array, this.firstIndex, this.array, increment, size);
         }
