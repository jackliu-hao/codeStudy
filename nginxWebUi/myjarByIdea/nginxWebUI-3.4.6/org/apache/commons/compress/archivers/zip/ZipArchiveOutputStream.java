...
public class ZipArchiveOutputStream extends ArchiveOutputStream {
   ...
   public ZipArchiveOutputStream(Path file, OpenOption... options) throws IOException {
      ...
      try {
         _channel = Files.newByteChannel(file, EnumSet.of(StandardOpenOption.CREATE, StandardOpenOption.WRITE, StandardOpenOption.READ, StandardOpenOption.TRUNCATE_EXISTING));
   ...
   private byte[] createCentralFileHeader(ZipArchiveEntry ze, ByteBuffer name, EntryMetaData entryMetaData, boolean needsZip64Extra) throws IOException {
      ...
      if (this.isSplitZip) {
         ...
         if (this.numberOfCDInDiskData.get(currentSplitSegment) == null) {
            ...
            extraLength = (Integer)this.numberOfCDInDiskData.get(currentSplitSegment);
            this.numberOfCDInDiskData.put(currentSplitSegment, extraLength + 1);
         }
      }
   ...
   protected void writeCentralDirectoryEnd() throws IOException {
      ...
      int numOfEntriesOnThisDisk = this.isSplitZip ? (this.numberOfCDInDiskData.get(numberOfThisDisk) == null ? 0 : (Integer)this.numberOfCDInDiskData.get(numberOfThisDisk)) : numberOfEntries;
   ...
   private void validateIfZip64IsNeededInEOCD() throws Zip64RequiredException {
      if (this.zip64Mode == Zip64Mode.Never) {
         ...
         if (numberOfThisDisk >= 65535) {
            ...
            int numOfEntriesOnThisDisk = this.numberOfCDInDiskData.get(numberOfThisDisk) == null ? 0 : (Integer)this.numberOfCDInDiskData.get(numberOfThisDisk);
   ...
   protected void writeZip64CentralDirectory() throws IOException {
      if (this.zip64Mode != Zip64Mode.Never) {
         ...
         if (this.hasUsedZip64) {
            ...
            int numOfEntriesOnThisDisk = this.isSplitZip ? (this.numberOfCDInDiskData.get(numberOfThisDisk) == null ? 0 : (Integer)this.numberOfCDInDiskData.get(numberOfThisDisk)) : this.entries.size();
   ...
   private static final class EntryMetaData {
      ...
      // $FF: synthetic method
      EntryMetaData(long x0, boolean x1, Object x2) {
         this(x0, x1);
      }
   }
