...
public class SevenZOutputFile implements Closeable {
   ...
   private CountingOutputStream setupFileOutputStream() throws IOException {
      if (this.files.isEmpty()) {
         ...
         for(Iterator var4 = this.getContentMethods((SevenZArchiveEntry)this.files.get(this.files.size() - 1)).iterator(); var4.hasNext(); first = false) {
            SevenZMethodConfiguration m = (SevenZMethodConfiguration)var4.next();
            if (!first) {
               CountingOutputStream cos = new CountingOutputStream((OutputStream)out);
               moreStreams.add(cos);
               out = cos;
            }

            out = Coders.addEncoder((OutputStream)out, m.getMethod(), m.getOptions());
         }
         ...
         return new CountingOutputStream((OutputStream)out) {
            public void write(int b) throws IOException {
               super.write(b);
   ...
   private void writePackInfo(DataOutput header) throws IOException {
      ...
      while(var2.hasNext()) {
         entry = (SevenZArchiveEntry)var2.next();
         if (entry.hasStream()) {
   ...
   private void writeFolder(DataOutput header, SevenZArchiveEntry entry) throws IOException {
      ...
      int numCoders = 0;
      Iterator var5 = this.getContentMethods(entry).iterator();

      while(var5.hasNext()) {
         SevenZMethodConfiguration m = (SevenZMethodConfiguration)var5.next();
         ++numCoders;
   ...
   private void writeFileEmptyStreams(DataOutput header) throws IOException {
      ...
      while(var3.hasNext()) {
         SevenZArchiveEntry entry = (SevenZArchiveEntry)var3.next();
         if (!entry.hasStream()) {
   ...
   private void writeFileEmptyFiles(DataOutput header) throws IOException {
      ...
      Iterator var5 = this.files.iterator();

      while(var5.hasNext()) {
         SevenZArchiveEntry file1 = (SevenZArchiveEntry)var5.next();
         if (!file1.hasStream()) {
   ...
   private void writeFileNames(DataOutput header) throws IOException {
      ...
      while(var4.hasNext()) {
         SevenZArchiveEntry entry = (SevenZArchiveEntry)var4.next();
   ...
   private void writeFileCTimes(DataOutput header) throws IOException {
      ...
      if (numCreationDates > 0) {
         ...
         while(var9.hasNext()) {
            SevenZArchiveEntry entry = (SevenZArchiveEntry)var9.next();
            if (entry.getHasCreationDate()) {
