...
public class X0017_StrongEncryptionHeader extends PKWareExtraHeader {
   ...
   public void parseFileFormat(byte[] data, int offset, int length) throws ZipException {
      ...
      if (this.rcount == 0L) {
         ...
         resize = ZipShort.getValue(data, offset + ivSize + 20 + erdSize);
         this.assertDynamicLengthFits("vSize", resize, ivSize + 22 + erdSize, length);
         if (resize < 4) {
            throw new ZipException("Invalid X0017_StrongEncryptionHeader: vSize " + resize + " is too small to hold CRC");
         }

         this.assertMinimalLength(offset + ivSize + 22 + erdSize, resize - 4);
         this.vData = Arrays.copyOfRange(data, offset + ivSize + 22 + erdSize, resize - 4);
         this.assertMinimalLength(offset + ivSize + 22 + erdSize + resize - 4, 4);
         this.vCRC32 = Arrays.copyOfRange(data, offset + ivSize + 22 + erdSize + resize - 4, 4);
      } else {
         ...
         resize = ZipShort.getValue(data, offset + ivSize + 24 + erdSize);
