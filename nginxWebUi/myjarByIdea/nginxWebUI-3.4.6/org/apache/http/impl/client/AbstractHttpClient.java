...
public abstract class AbstractHttpClient extends CloseableHttpClient {
   ...
   protected ClientConnectionManager createClientConnectionManager() {
      ...
      return (ClientConnectionManager)connManager;
   }
   ...
   private synchronized HttpProcessor getProtocolProcessor() {
      if (this.protocolProcessor == null) {
         ...
         int resc;
         for(resc = 0; resc < reqc; ++resc) {
            reqinterceptors[resc] = proc.getRequestInterceptor(resc);
         }

         resc = proc.getResponseInterceptorCount();
