...
public interface Messages extends BasicLogger {
   Messages msg = (Messages)Logger.getMessageLogger(Messages.class, "org.xnio");
   Messages futureMsg = (Messages)Logger.getMessageLogger(Messages.class, "org.xnio.future");
   Messages optionParseMsg = (Messages)Logger.getMessageLogger(Messages.class, "org.xnio.option.parse");
   Messages closeMsg = (Messages)Logger.getMessageLogger(Messages.class, "org.xnio.safe-close");
   Messages listenerMsg = (Messages)Logger.getMessageLogger(Messages.class, "org.xnio.listener");
   ...
   IllegalArgumentException minRange(String var1, int var2);

   @Message(
      id = 2,
      value = "Unsupported socket address %s"
   )
   IllegalArgumentException badSockType(Class<? extends SocketAddress> var1);

   @Message(
      id = 3,
      value = "Method parameter '%s' contains disallowed null value at index %d"
   )
   IllegalArgumentException nullArrayIndex(String var1, int var2);

   @Message(
      id = 4,
      value = "Bind address %s is not the same type as destination address %s"
   )
   IllegalArgumentException mismatchSockType(Class<? extends SocketAddress> var1, Class<? extends SocketAddress> var2);

   @Message(
      id = 5,
      value = "No such field named \"%s\" in %s"
   )
   IllegalArgumentException noField(String var1, Class<?> var2);

   @Message(
      id = 6,
      value = "Class \"%s\" not found in %s"
   )
   IllegalArgumentException optionClassNotFound(String var1, ClassLoader var2);

   @Message(
      id = 7,
      value = "Field named \"%s\" is not accessible (public) in %s"
   )
   IllegalArgumentException fieldNotAccessible(String var1, Class<?> var2);

   @Message(
      id = 8,
      value = "Field named \"%s\" is not static in %s"
   )
   IllegalArgumentException fieldNotStatic(String var1, Class<?> var2);
   ...
   void invalidOptionInProperty(String var1, String var2, Throwable var3);
   ...
   IllegalArgumentException invalidOptionPropertyFormat(String var1);

   @Message(
      id = 22,
      value = "Class %s not found"
   )
   IllegalArgumentException classNotFound(String var1, @Cause ClassNotFoundException var2);

   @Message(
      id = 23,
      value = "Class %s is not an instance of %s"
   )
   IllegalArgumentException classNotInstance(String var1, Class<?> var2);

   @Message(
      id = 24,
      value = "Invalid option name '%s'"
   )
   IllegalArgumentException invalidOptionName(String var1);

   @Message(
      id = 25,
      value = "Invalid null option '%s'"
   )
   IllegalArgumentException invalidNullOption(String var1);
   ...
   IllegalArgumentException invalidQop(String var1);

   @Message(
      id = 34,
      value = "Failed to instantiate %s"
   )
   IllegalArgumentException cantInstantiate(Class<?> var1, @Cause Throwable var2);

   @Message(
      id = 35,
      value = "Stream channel was accessed concurrently"
   )
   ...
   IllegalArgumentException mismatchAddressType(Class<? extends InetAddress> var1, Class<? extends InetAddress> var2);

   @Message(
      id = 41,
      value = "'%s' is not a valid Strength value"
   )
   IllegalArgumentException invalidStrength(String var1);

   @Message(
      id = 42,
      value = "Cannot add unresolved address '%s'"
   )
   IllegalArgumentException addressUnresolved(InetSocketAddress var1);
   ...
   IllegalArgumentException invalidURLScheme(String var1);
   ...
   IOException unexpectedWrapResult(SSLEngineResult.Status var1);

   @Message(
      id = 304,
      value = "Unexpected handshake status: %s"
   )
   IOException unexpectedHandshakeStatus(SSLEngineResult.HandshakeStatus var1);

   @Message(
      id = 305,
      value = "Unexpected unwrap result status: %s"
   )
   IOException unexpectedUnwrapResult(SSLEngineResult.Status var1);
   ...
   @Message(
      id = 307,
      value = "Failed to close ssl engine when handling exception %s"
   )
   @LogMessage(
      level = Logger.Level.WARN
   )
   void failedToCloseSSLEngine(@Cause Throwable var1, Exception var2);
   ...
   @Message(
      id = 802,
      value = "Write past the end of a fixed-length channel"
   )
   ...
   FixedLengthUnderflowException fixedUnderflow(long var1);

   @Message(
      id = 804,
      value = "Received an invalid message length of %d"
   )
   IOException recvInvalidMsgLength(int var1);
   ...
   @Message(
      id = 1003,
      value = "Running IoFuture notifier %s (with attachment %s) failed"
   )
   @LogMessage(
      level = Logger.Level.WARN
   )
   void notifierFailed(@Cause Throwable var1, IoFuture.Notifier<?, ?> var2, Object var3);
   ...
   void listenerException(@Cause Throwable var1);
   ...
   void exceptionHandlerException(@Cause Throwable var1);

   @Message(
      id = 1009,
      value = "Failed to accept a connection on %s"
   )
   @LogMessage(
      level = Logger.Level.ERROR
   )
   void acceptFailed(AcceptingChannel<? extends ConnectedChannel> var1, @Cause IOException var2);

   @Message(
      id = 1010,
      value = "Failed to submit task to executor: (closing %s)"
   )
   @LogMessage(
      level = Logger.Level.ERROR
   )
   void executorSubmitFailed(@Cause RejectedExecutionException var1, Channel var2);

   @Message("Closing resource %s")
   @LogMessage(
      level = Logger.Level.TRACE
   )
   void closingResource(Object var1);

   @Message("Closing resource %s failed")
   @LogMessage(
      level = Logger.Level.TRACE
   )
   void resourceCloseFailed(@Cause Throwable var1, Object var2);

   @Message("Shutting down reads on %s failed")
   @LogMessage(
      level = Logger.Level.TRACE
   )
   void resourceReadShutdownFailed(@Cause Throwable var1, Object var2);

   @Message("Expanded buffer enabled due to overflow with empty buffer, expanded buffer size is %s")
   @LogMessage(
      level = Logger.Level.TRACE
   )
   void expandedSslBufferEnabled(int var1);
}
