...
public final class SaslUnwrappingConduit extends AbstractMessageSinkConduit<MessageSinkConduit> implements MessageSinkConduit {
   ...
   public boolean send(ByteBuffer src) throws IOException {
      if (!this.doSend()) {
         ...
         ByteBuffer wrapped = ByteBuffer.wrap(this.wrapper.unwrap(src));
   ...
   public boolean send(ByteBuffer[] srcs, int offs, int len) throws IOException {
      if (!this.doSend()) {
         ...
         byte[] bytes = Buffers.take(srcs, offs, len);
         ByteBuffer wrapped = ByteBuffer.wrap(this.wrapper.unwrap(bytes));
         if (!((MessageSinkConduit)this.next).send(wrapped)) {
            this.buffer = wrapped;
         }
